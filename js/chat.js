function chatApp() {
    return {
        ...
        async sendMessage() {
            ...
            // Play message sound
            if (window.NotificationSystem && this.settings.soundEnabled) {
                NotificationSystem.playSound('message');
            }
            ...
            try {
                ...
                const response = {
                    role: 'assistant',
                    content: this.generateResponse(question, files),
                    timestamp: Date.now()
                };
        ...
        generateResponse(question, files) {
            if (files.length > 0) {
                return `📎 I received your message: "${question}" along with ${files.length} file(s). How can I help you with these?`;
            }
