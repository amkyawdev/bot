// js/chat.js - Complete Chat System with API.js Connection
...
class ChatSystem {
    ...
    // FIXED: API call ကို မှန်ကန်အောင်ပြင်ထားတယ်
    async callAPI(message) {
        ...
        try {
            ...
            // FIXED: error ဖြစ်ရင် error message ပြမယ်
            if (!result || !result.success) {
                throw new Error(result?.data?.response || 'API returned unsuccessful response');
            }
    ...
    async sendMessage() {
        ...
        try {
            // CALL API
            console.log('🔍 Calling API via api.js...');
            const apiResult = await this.callAPI(message);

            // FIXED: response ကို မှန်ကန်စွာထုတ်ယူ
            const response = apiResult.data?.response || 'No response from API';
            ...
            this.addMessage('system', `❌ Error: ${error.message || 'Failed to get response from API'}`);
...
console.log('✅ Chat.js loaded - API.js CONNECTED VERSION');